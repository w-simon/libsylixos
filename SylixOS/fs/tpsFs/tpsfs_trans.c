...
TPS_RESULT  tpsFsBtreeTransInit (PTPS_SUPER_BLOCK psb)
{
    ...
    ptranssb->TSB_ui64TransSecCnt    = TPS_TRANS_ARRSIZE >> (psb->SB_uiSectorShift - TPS_TRAN_SHIFT);
	if (((psb->SB_ui64LogBlkCnt << psb->SB_uiBlkShift) >> psb->SB_uiSectorShift) 
                                       ...
                                       UINT64       ui64SecCnt)
{
	...
	if ((ptrdata->TD_uiSecAreaCnt >= (TPS_TRANS_MAXAREA - 1)) ||
		((ptrans->TRANS_uiDataSecCnt + ui64SecCnt) > 
		 (psb->SB_ptranssb->TSB_ui64TransDataStart +
		  psb->SB_ptranssb->TSB_ui64TransDataCnt - TPS_TRANS_REV_DATASEC))) {
