...
** 输　入  : name      msg 名
**           flags     open flags
**           ...       mode_t mode, size_t size, size_t atomic_len
** 输　出  : 文件描述符
** 全局变量:
** 调用模块:
*********************************************************************************************************/
...
int bmsgfd (const char *name, int flags, ...)
{
    ...
    struct bmsg_param   param = {0, 0, 0, 0};
    ...
    if (flags & O_CREAT) {
        ...
        if (flags & BMSGFD_SETBUFFER) {
            param.total_size  = va_arg(valist, UINT32);
            param.atomic_size = va_arg(valist, UINT32);
        }
        va_end(valist);
    }
